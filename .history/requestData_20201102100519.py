...
for e in j['confirmed']:
    del e["Lat"]
    del e["Long"]
    countryRegion = f"{e['Country/Region']}\
...
for e in listCountryConfirm:
    ...
    summaryDict.update({country: {'inDayconfirm': {}}})
    summaryDict[country]['confirm'].update(e)
    valueList = summaryDict[country]['confirm'].values()
    keyList = summaryDict[country]['confirm'].keys()
...
for e in listCountryDeaths:
    country = e['country']
    del e['country']
    if country in summaryDict.keys():
